import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, RecaptchaVerifier, signInWithPhoneNumber } from "https://www.gstatic.com/firebasejs/9.22.0/firebase-auth.js";
...
$(document).ready(function () {
    ...
    $('#register').on('click', async function (e) {
        ...
        if (!validatePassword(password)) {
            alert("비밀번호는 최소 8자 이상이어야 하며, 대문자 또는 소문자, 숫자, 특수문자를 포함해야 합니다.");
            return;
        }
        ...
        try {
            ...
            if (error.code === 'auth/email-already-in-use') {
                ...
                console.error("오류가 발생했습니다. 문제가 지속되면 고객 센터에 문의해주세요:", error.message);
            }
        }
    });
    ...
    $('#login').on('click', async function (e) {
        ...
        try {
            ...
            console.log("로그인 성공!");
            window.location.href = 'login/ko.html';  // 로그인 후 이동할 페이지
        } catch (error) {
            if (error.code === 'auth/wrong-password') {
                alert("비밀번호가 일치하지 않습니다.");
            } else {
                alert("오류가 발생했습니다. 문제가 지속되면 고객 센터에 문의해주세요.");
            }
        }
    });
});